...
def test_claude_source_file(result_claude: ParseResult) -> None:
    assert result_claude.source_file is not None
    assert "claude_desktop" in result_claude.source_file


def test_claude_no_warnings(result_claude: ParseResult) -> None:
...
@pytest.fixture(scope="module")
def result_github() -> ParseResult:
    return parse_mcp_file(GITHUB_BRAVE)


def test_github_nodes_count(result_github: ParseResult) -> None:
    # 4 github tools + 2 brave tools = 6
    assert len(result_github.nodes) == 6


def test_github_server_names(result_github: ParseResult) -> None:
    servers = {n.server_name for n in result_github.nodes}
    assert "github" in servers
    assert "brave-search" in servers


def test_github_create_issue_caps(result_github: ParseResult) -> None:
    node = next(n for n in result_github.nodes if n.tool_name == "create_issue")
    assert "write" in node.caps


def test_github_get_issue_caps(result_github: ParseResult) -> None:
    node = next(n for n in result_github.nodes if n.tool_name == "get_issue")
    assert "read" in node.caps


def test_github_push_files_caps(result_github: ParseResult) -> None:
    node = next(n for n in result_github.nodes if n.tool_name == "push_files")
    assert "write" in node.caps


def test_github_search_caps(result_github: ParseResult) -> None:
...
def test_github_def_hashes_unique(result_github: ParseResult) -> None:
...
def test_github_trust_edges(result_github: ParseResult) -> None:
    trust = [e for e in result_github.edges if e.edge_type == "trust"]
    assert len(trust) > 0


def test_github_data_edges(result_github: ParseResult) -> None:
    data = [e for e in result_github.edges if e.edge_type == "data"]
    assert len(data) > 0


def test_github_no_warnings(result_github: ParseResult) -> None:
...
@pytest.fixture(scope="module")
def result_nonstandard() -> ParseResult:
    return parse_mcp_file(NONSTANDARD)


def test_nonstandard_does_not_crash(result_nonstandard: ParseResult) -> None:
    # Parser must not raise even with malformed entries
    assert isinstance(result_nonstandard, ParseResult)


def test_nonstandard_valid_tools_extracted(result_nonstandard: ParseResult) -> None:
...
def test_nonstandard_alt_name_key(result_nonstandard: ParseResult) -> None:
    # "tool_name" key should be accepted
    names = {n.tool_name for n in result_nonstandard.nodes}
    assert "alt_name_tool" in names


def test_nonstandard_valid_in_mixed_list(result_nonstandard: ParseResult) -> None:
    # Tool nested inside a list with non-dict entries should be extracted
    names = {n.tool_name for n in result_nonstandard.nodes}
    assert "valid_in_mixed_list" in names


def test_nonstandard_malformed_skipped(result_nonstandard: ParseResult) -> None:
    names = {n.tool_name for n in result_nonstandard.nodes}
    assert "malformed_no_name" not in names


def test_nonstandard_warnings_issued(result_nonstandard: ParseResult) -> None:
    # Malformed entries should produce warnings, not silently pass
    assert len(result_nonstandard.warnings) > 0


def test_nonstandard_server_names_preserved(result_nonstandard: ParseResult) -> None:
    servers = {n.server_name for n in result_nonstandard.nodes}
    assert "search-engine" in servers
    assert "code-runner" in servers


def test_nonstandard_execute_code_caps(result_nonstandard: ParseResult) -> None:
    node = next(n for n in result_nonstandard.nodes if n.tool_name == "execute_code")
    assert "exec" in node.caps


def test_nonstandard_def_hash_present(result_nonstandard: ParseResult) -> None:
